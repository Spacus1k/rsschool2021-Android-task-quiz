...
class ThirdTicketFragment : Fragment(), IQuestionNumber {
    ...
    override fun onCreateView(
        ...
        activity?.let {
            setTheme(
                it,
                R.style.Theme_Quiz_ThirdDeepGreen,
                R.color.deep_green_100_dark
            )
        }
    ...
    private fun checkPreviousAnswer() {
        binding?.apply {
            if (quizInteractor.getAnswerIndex(this@ThirdTicketFragment.currentQuestionNumber) != null) {
                this.nextButton.isEnabled = true

                when (quizInteractor.getAnswerIndex(this@ThirdTicketFragment.currentQuestionNumber)) {
